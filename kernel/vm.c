...
void printpgtbl(pagetable_t pgtbl, int level)
{
  for(int i = 0; i < 512; i++)
  {
    pte_t pte = pgtbl[i];
...
void
proc_kvmmap(pagetable_t p, uint64 va, uint64 pa, uint64 sz, int perm)
{
...
void 
uvmcopy_n(pagetable_t u, pagetable_t k, uint64 start, uint64 end)
{
  pte_t *usr;
  pte_t *knl;
  for(uint i = start; i < end; i+=PGSIZE)
  {
    usr = walk(u, i, 0);
    knl = walk(k, i ,1);
    *knl = (*usr)&(~PTE_U);
